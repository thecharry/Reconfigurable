...
% % 总共 48 个变量
% n =8;
% lb = repmat([-Y_max, -Z_max, 0,    0], 1, n);
% ub = repmat([ Y_max,  Z_max, 2*pi, pi/2], 1, n);
...
% tic;
% % 变量维度设为 48
% [x_opt, fval] = ga(@(x) Optimal_config_free(x, params,n), 4*n, [], [], [], [], lb, ub, @(x) Physical_constraints_free(x, params,n), options_ga);
% toc;

% [B_opt, r_opt] = Thruster_reconfig_free(x_opt, params,n);
% save('Optim_config_data_GlobalFree.mat', 'B_opt', 'r_opt', 'fval', 'x_opt');
% disp('优化完成！理论最优构型已保存。');


% %% 推力器布局配置 (全独立计算)
% function [B_all, r] = Thruster_reconfig_free(x, params,n)
%     r = zeros(3, n);
%     d = zeros(3, n);
%     B_all = zeros(6, n);
...
% %% 最优布局参数目标函数 (带软惩罚项)
% function J = Optimal_config_free(x, params,n)
%     [B_all, ~] = Thruster_reconfig_free(x, params,n);
%     w = 0.5;
...
%         v_Jc1 = max(0, 0 - Jc_eval(1));
%         v_Jc2 = max(0, 0 - Jc_eval(2));
...
% function [c, ceq] = Physical_constraints_free(x, params,n)
%     [~, r] = Thruster_reconfig_free(x, params,n);
%     D_min = 0.1; % 最小安装间距
